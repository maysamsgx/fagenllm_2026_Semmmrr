...
def _write_forecast(accounts: list, inflows: float, outflows: float) -> None:
    ...
    # Batch delete and insert to avoid hitting sequential request limits/timeouts
    forecast_dates = [r["forecast_date"] for r in rows]
    
    try:
        # 1. Batch Delete
        supabase.table("cash_flow_forecasts") \
            .delete() \
            .in_("forecast_date", forecast_dates) \
            .eq("cash_account_id", account_id) \
            .execute()
            
        # 2. Batch Insert
        # Strip 'net_position' (generated by DB) and 'projected_balance' (missing from current schema)
        # to avoid 400 Bad Request errors.
        rows_to_insert = []
        for r in rows:
            clean_row = {k: v for k, v in r.items() if k not in ("net_position", "projected_balance")}
            rows_to_insert.append(clean_row)
            
        if rows_to_insert:
            supabase.table("cash_flow_forecasts").insert(rows_to_insert).execute()
            
    except Exception as e:
        import logging
        logging.error(f"Failed to persist cash forecast: {e}")
